...
class SparklineChart extends BaseElement {
    ...
    redrawPoint() {
        ...
        const scoreHoverRectWidth = this.scoreElement_.getBoundingClientRect()
            .width;
        const x = Math.max(PADDING, Math.min(this.width_ - scoreHoverRectWidth - 2 * PADDING, this.point_.x - scoreHoverRectWidth / 2));
        const y = Math.max(-1 * (this.height_ / 2), Math.min(this.height_ / 2 - PADDING - 10, this.point_.y - 10));
        ...
        this.scoreDateText_.setAttribute("x", String(scoreHoverRectWidth / 2 - dateTextWidth / 2));
        this.scoreValueText_.setAttribute("x", String(scoreHoverRectWidth / 2 - scoreTextWidth / 2));
    ...
    setPrevPoint_() {
        const cursorX = +this.cursorElement_.getAttribute("x1");
        const currentPointIdx = this.datapoints.findIndex((entry) => entry.x === cursorX);
        const prevPoint = currentPointIdx === -1
    ...
    processValues(values) {
        ...
        const offset = values.length > 1 ? Math.floor(this.width_ / (values.length - 1)) : 0;
        const datapoints = [];
        const paths = [];
    ...
    render() {
        ...
        const groupTransform = `translate(${pixelBuffer},${pixelBuffer + this.topPadding})`;
    ...
    firstUpdated() {
        ...
        this.scoreValueText_ = this.scoreElement_.querySelector(".sl-caption--value");
