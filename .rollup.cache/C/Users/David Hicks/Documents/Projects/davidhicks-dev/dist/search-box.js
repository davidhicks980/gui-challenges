...
customElements.define("web-side-nav", class extends LitElement {
    ...
    onStateChanged({ currentUrl: currentUrl } = {
        currentUrl: null,
    }) {
        const { isSideNavExpanded } = this.getState();
        if (isSideNavExpanded !== this.expanded &&
            ((this.expanded = isSideNavExpanded), currentUrl)) {
            currentUrl = ((currentUrl = currentUrl.replace(/"/g, '\\"')).match(/^\/\w+\//) || [""])[0];
            const e = this.querySelector("[active]"), n = this.querySelector(`[href="${currentUrl}"]`);
            if (e === n)
                return;
    ...
    onTransitionEnd() {
        if (((this.animatable = false), this.expanded_))
            this.focus();
